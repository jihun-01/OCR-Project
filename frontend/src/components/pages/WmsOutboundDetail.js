...
const WmsOutboundDetail = () => {
  const { id } = useParams();
  const order = mockData.find((item) => item.id === id);

  // 주문을 찾지 못한 경우
  if (!order) {
    return (
      <div className="pt-16 min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 font-sans">
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-12">
            ...
            <p className="text-gray-600 mb-6">주문번호 '{id}'에 해당하는 정보가 없습니다.</p>
            <Link 
              to="/wms" 
    ...
    <div className="pt-16 min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <h1 className="mt-4 text-3xl font-bold text-gray-900">주문 상세 정보</h1>
          <Link 
            to="/wms" 
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
          ...
          <div className="p-8 lg:p-12 grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b border-gray-200 pb-3">주문 정보</h2>
              <div className="space-y-4">
                <div className="flex justify-between">
                  <span className="font-semibold text-gray-700">주문번호:</span>
                ...
                <div className="flex justify-between">
                  <span className="font-semibold text-gray-700">운송장 번호:</span>
                  <span className="font-mono bg-gray-100 px-2 py-1 rounded">{order.trackingNumber}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-gray-700">주문일자:</span>
                  <span>{order.date}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-gray-700">출고 상태:</span>
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                    order.status === '출고 완료' ? 'bg-green-100 text-green-800' :
                    order.status === '출고 대기' ? 'bg-yellow-100 text-yellow-800' :
            ...
            {/* 주문 상품 정보 */}
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b border-gray-200 pb-3">주문 상품</h2>
              <div className="bg-gray-50 rounded-xl p-6 space-y-4">
                {order.items.map((item, index) => (
                  <div key={index} className="flex justify-between items-center border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
                    <div>
                      <span className="font-semibold text-gray-900">{item.name}</span>
                      <span className="text-gray-600 ml-2">x {item.quantity}개</span>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold text-gray-900">{(item.quantity * item.price).toLocaleString()}원</div>
                      <div className="text-sm text-gray-500">단가: {item.price.toLocaleString()}원</div>
                    </div>
                  </div>
                ))}
                <div className="flex justify-between items-center border-t border-gray-300 pt-4 font-bold text-lg">
                  <span className="text-blue-700">총 금액</span>
          ...
          <div className="px-8 pb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b border-gray-200 pb-3">출고 진행 상태</h2>
