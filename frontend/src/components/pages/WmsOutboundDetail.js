...
const WmsOutboundDetail = ({ id: propId, order: propOrder, onClose }) => {
  const params = useParams();
  const id = propId || params.id;
  const order = propOrder || mockData.find((item) => item.id === id);
            ...
            <div className="p-8 lg:p-12 grid lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between h-28 flex-wrap">
                    <span className="dark:text-white font-semibold text-gray-700">주소:</span>
                    <span className="dark:text-white">{order.address}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {order.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-center border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
                      ...
                      <div className="text-right">
                        <div className="dark:text-white font-semibold text-gray-900">{(item.quantity * item.price).toLocaleString()}원</div>
                        <div className="dark:text-white text-sm text-gray-500">단가: {item.price.toLocaleString()}원</div>
                      </div>
                    </div>
                  ))}
